...
- `sextuples` (array; see "Additionally extract defect-resolution sextuples" below)
...
- The `sextuples` key is optional. Omit it (or pass an empty array) when the
  rollout contains no genuinely resolved defect moments.
- No additional keys beyond those listed above.
- No prose outside JSON.

============================================================
Additionally extract defect-resolution sextuples
============================================================

When the rollout contains concrete defect-resolution moments (a bug was
identified and fixed, a failing test was made to pass, an error was
diagnosed and resolved), also emit a `sextuples` array alongside the
existing fields. Each entry must follow this schema:

```json
{
  "sextuples": [
    {
      "keywords": ["auth", "401", "token-refresh"],
      "problem": "Session requests intermittently return 401 after token lifetime exceeds 1 hour",
      "root_cause": "Access token was cached in memory without honoring the `exp` claim; refresh logic only ran on explicit 401 response rather than proactively.",
      "solution": "Added a proactive refresh timer that fires 5 minutes before `exp`; cached token is invalidated on clock skew > 30s. See commit <sha> for implementation."
    }
  ]
}
```

Include at most 5 sextuples per rollout. Only extract genuinely
resolved defects — skip aborted attempts or abandoned threads. Each
entry's problem+root_cause+solution must stand on its own without
requiring the original rollout context to understand.
